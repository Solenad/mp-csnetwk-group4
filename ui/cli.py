...
def cmd_help(_args):
    ...
    commands = [
        ...
        "groups                            - List your groups or group members",
        "group_create <id> <name> [members]- Create a new group",
        "group_update <id> <add|remove> <members> - Update group membership",
        "group_message <id> <message>      - Send message to a group",
        "ttt invite <username> <X|O>       - Invite to TicTacToe",
        ...
        "test token                        - Test token validation",
        "test all                          - Run all protocol tests",
        ...
        "y                                 - Accept pending file transfer (when prompted)",
        "n                                 - Reject pending file transfer (when prompted)",
    ]
    for cmd in commands:
        if cmd:
            print(f" - {cmd}")
...
def cmd_group_update(args):
    ...
    if len(args) < 4:
        print_error(
            "Usage: group_update <group_id> add <member1,member2,...> | remove <member1,member2,...>"
        )
        return True
